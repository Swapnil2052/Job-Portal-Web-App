...
public class UsersController {
    ...
    public String userRegistration(@Valid Users users, RedirectAttributes redirectAttributes){
        ...
        if(optionalUsers.isPresent()){
            /*redirect attributes can pass the data from one request to another and since model attributes can not,
             we need to use here redirectAttribute coz we need to pass "error" values to redirected request */
            redirectAttributes.addFlashAttribute("error","Email already registered, try to login or register with other email");
